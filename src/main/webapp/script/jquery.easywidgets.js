...
(function($){
    ...
    $.fn.EasyWidgets.defaults = {
        ...
        selectors : {
            ...
            savePanel : '.widget-savepanel',

            saveLink : '.widget-savelink2',

            cancelLink : '.widget-cancellink2',

            // Class identifier for a Widget close link
            closeLink : '.widget-closelink',

            // Class identifier for a Widget newTab link
            newTabLink : '.widget-newtablink',
            ...
            dropdownMenu : '.widget-dropdown-menu',

            dropdownIcon : '.widget-dropdown-icon2'
        ...
        i18n : {
            ...
            editText : 'Edit config',

            viewConfigText : 'View config',

            //Widget Open in a new Tab link text
            openNewTabText : 'Open in a new tab',
            ...
            //Widget Open in a new Tab link title
            openNewTabTitle : 'Open in a new tab',

            viewConfigTitle : 'View configuration',
...
//            widget.find(s.selectors.header).append(menuWrap);
            widget.find(".widget-gradient").append(menuWrap);
            ...
            AddWidgetSavePanel(widget, widgetMenu, s);
            AddMenuIcon(widget, widgetMenu, s);
            var dropdownMenu = widget.find(s.selectors.dropdownMenu);
            AddWidgetCollapseLink(widget, widgetMenu, s);
            AddWidgetEditLink(widget, dropdownMenu, s);
            AddWidgetNewTabLink(widget, dropdownMenu, s);
            AddWidgetRemoveLink(widget, dropdownMenu, s);
//            AddWidgetCustomLink(widget, dropdownMenu, s);
            HideMenuIconIfNecessary(widget, dropdownMenu, s);
            return true;
        }else{
            return false;
        }
    ...
    function AddMenuIcon(widget, widgetMenu, settings) {
        var max = $("<i title='Full Screen' class='icon-fullscreen' style='cursor:pointer;'></i>");
        max.click(function() {
            if(widget.hasClass('max')) {
                widget.removeClass('max');
                widget.find("iframe").css('height','450px');
                widget.find("iframe").contents().find("iframe").css('height','450px');
                $(".mask-max").remove();
                max.attr('title', 'Full Screen');
            } else {
                widget.addClass('max');
                $(document.body).append("<div class='t30 mask-max'></div>");
                window.scrollTo(0, 0);
                max.attr('title', 'Restore Size');
                widget.find("iframe").css('height','745px');
                widget.find("iframe").contents().find("iframe").css('height','745px');
            }
        });



        var icon = $("<i style='font-size:auto; cursor:pointer; margin: 2px 5px 0 0' class='icon-cog "  + settings.selectors.dropdownIcon.replace(/\./, '')  + "'></i>");
//        var icon = $("<i class='"  + settings.selectors.dropdownIcon.replace(/\./, '')  + "'></i>");

        var dropdown = $('<ul class="' + settings.selectors.dropdownMenu.replace(/\./, '') + '" style="display: block; position: absolute;"></ul>');
        icon.append(dropdown);
        dropdown.hide();
        icon.hover(function() {
            var off = icon.offset();
            var w = icon.outerWidth();
            var h = icon.outerHeight();
            var top = off.top + h + 2;
            var dropdownWidth = dropdown.outerWidth();
            dropdown.stop(true, true).show();
            var left = off.left + w - dropdownWidth;
            dropdown.offset({top: top, left: left});
        }, function() {
            dropdown.stop(true, true).delay(250).slideUp(200);
        });
        widgetMenu.append(icon);
        widgetMenu.append(max);
    }

    function HideMenuIconIfNecessary(widget, widgetMenu, settings) {
        var icon = widget.find(settings.selectors.dropdownIcon);
        var dropdownMenu = widget.find(settings.selectors.dropdownMenu);
        var li_count = dropdownMenu.find("li").length;
        if (li_count <= 0) {
            icon.hide();
        }
        return;
    ...
    function AddWidgetSavePanel(widget, widgetMenu, settings) {
        var s = settings;
        var save = '';
        var cancel = '';
        var savePanel = '<span class="' + s.selectors.savePanel.replace(/\./, '') + '"></span>';
        if (widget.hasClass(s.options.editable) || widget.hasClass(s.options.viewable)) {
            if (widget.hasClass(s.options.editable)) {
                save = $('<a style="margin-right: 5px; cursor: pointer;" title="' + s.i18n.saveTitle + '" class="icon-save ' + s.selectors.saveLink.replace(/\./, '') + '"></a>');
            } else {
                save = $('<a style="margin-right: 5px; cursor: pointer;" title="' + s.i18n.saveTitle + '" class="icon-save ' + s.selectors.saveLink.replace(/\./, '') + '"></a>');
            }
            cancel = $('<a style="margin-right: 5px; cursor: pointer;" title="' + s.i18n.cancelEditTitle + '" class="icon-mail-reply ' + s.selectors.cancelLink.replace(/\./, '') + '"></a>');
            savePanel = $(savePanel);
            savePanel.appendTo(widgetMenu);

            savePanel.hide();
            cancel.appendTo(savePanel);

            if (widget.hasClass(s.options.editable)) {
                save.appendTo(savePanel);
                save.click(function(e){
                    var link = $(this);
                    var widget = link.parents(s.selectors.widget);
                    var editbox = widget.find(s.selectors.editbox);
                    var content = widget.find(s.selectors.content);
                    var dropdownIcon = widget.find(s.selectors.dropdownIcon)
                    var canSave = true;

                    if($.isFunction(s.callbacks.onSaveEditQuery)){
                        canSave = s.callbacks.onSaveEditQuery(link, widget);
                    }
                    if (canSave) {
                        savePanel.hide();
                        dropdownIcon.show();
                        ApplyEffect(
                            editbox,
                            s.effects.widgetSaveEdit,
                            s.effects.effectDuration,
                            false
                        );
                        ApplyEffect(
                            content,
                            s.effects.widgetShow, s.effects.effectDuration, true
                        );
                        if($.isFunction(s.callbacks.onSaveEdit)){
                            s.callbacks.onSaveEdit(link, widget);
                        }
                    }
                });
            }
    ...
    function AddWidgetCustomLink(widget, widgetMenu, settings) {
        var s = settings;
        mail = MenuLink("Mail this widget", "Mail this widget", ".widget-maillink");
        mail = $(mail);
        mail.appendTo(widgetMenu);
    ...
    function AddWidgetEditLink(widget, widgetMenu, settings){
        ...
        if(widget.hasClass(s.options.editable) || widget.hasClass(s.options.viewable)){
            if (widget.hasClass(s.options.editable)) {
                ...
                link = MenuLink(
                    s.i18n.viewConfigText,
                    s.i18n.viewConfigTitle,
                    s.selectors.editLink
                );
            }
            ...
            $(link).mousedown(function(e){
                ...
                function(){
                    ...
                    var widget = link.parents(s.selectors.widget);
                    var savePanel = widget.find(s.selectors.savePanel);
                    var editbox = widget.find(s.selectors.editbox);
                    var content = widget.find(s.selectors.content);
                    ...
                    if(editboxVisible){
                        ...
                        if(canShow){
                            ...
                            ApplyEffect(
                                content,
                                "none", s.effects.effectDuration, false
                            );
                ...
                e.stopPropagation();
                    ...
                    if(!widget.hasClass(s.options.closeConfirm)
                        ...
                        if(canRemove){
                            ...
                            if(widget.hasClass('max')){
                                $(".mask-max").remove(); // fix bug to avoid full size remove issue
                            }
                            widget.remove();
    ...
    /**
     * Prepare a widget newTab menu link
     *
     * @access private
     * @see PrepareWidgetBehaviour()
     * @param widget jQuery object with a widget encapsulation
     ...
     *
     */
    function AddWidgetNewTabLink(widget, widgetMenu, settings){
        var s = settings;
        var link = '';
        link = MenuLink(
            s.i18n.openNewTabText,
            s.i18n.openNewTabTitle,
            s.selectors.newTabLink
        );
        $(link).mousedown(function(e){
            e.stopPropagation();
        }).click(function(){
            ...
            window.open(localhostPath+'/qd/widget/view/'+widget.attr('id').substr(9)+'?isOpenNewTab=true','_blank');
        }).appendTo(widgetMenu);
    ...
    function MenuLink(text, title, aClass){
        var l = '<li><a href="#" title="TITLE" class="CLASS">TEXT</a></li>';
    ...
    function DropdownMenuLink(text, title, aClass) {
        var l = '<ul class="CLASS" style="display: block; position: absolute; border: 1px solid"></ul>';
        return l;
    }
